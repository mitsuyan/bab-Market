...
`)}
  ...
  `}async function generateImportFnScriptCode(options){let stories=await listStories(options);return(await toImportFn(stories)).trim()}var import_core_common6=require("@storybook/core-common");var absoluteFilesToImport=async(files,name,normalizePath)=>files.map((el,i)=>`import ${name?`* as ${name}_${i} from `:""}'/@fs/${normalizePath(el)}'`).join(`
`);async function generateVirtualStoryEntryCode(options){let{normalizePath}=await import("vite"),storyEntries=await listStories(options),resolveMap=storyEntries.reduce((prev,entry)=>({...prev,[entry]:entry.replace(slash(process.cwd()),".")}),{}),modules=storyEntries.map((entry,i)=>`${JSON.stringify(entry)}: story_${i}`).join(",");return`
