...
`)}
  ...
  `}async function generateImportFnScriptCode(options){let stories=await listStories(options);return (await toImportFn(stories)).trim()}var absoluteFilesToImport=async(files,name,normalizePath)=>files.map((el,i)=>`import ${name?`* as ${name}_${i} from `:""}'/@fs/${normalizePath(el)}'`).join(`
