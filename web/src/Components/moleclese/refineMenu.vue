<template>
    <div class="container"
        v-if="categoryId === 1 || categoryId === 2 || categoryId === 3 || categoryId === 4 || categoryId === 5 || categoryId === 6 || categoryId === 8 || categoryId === 9">
        <menuText level="level1" label="すべて" />
        <div v-for="item, index in filteredItems" :key="index"
            :class="{ 'border-top': index === 0, 'border-bottom': index !== items.length - 1 }">
            <checkInput checkBoxLevel="checkBoxGray" class="containerLevel1" :majorId="item.majorId" :id="item.id"
                :checked="item.checked" @change="handleCheckboxChange">
    ...
    <div class="container" v-else-if="categoryId === false">
        <menuText level="level1" label="すべて" />
        <div v-for="item, index in filteredItems" :key="index"
            :class="{ 'border-top': index === 0, 'border-bottom': index !== items.length - 1, [menu]: true }">
            ...
            <toggleButton :id="item.id" @isToggle="toggleChange">
                <div class="majorCategoryMenu" :id="item.id">
                    ...
                    <div class="minorCategoryItemColor" v-else-if="item.id === 7">
                        <div v-for=" minorCategory, index in item.value" :key="index" class="minorCategoryItemColorCircle">
                            <colorCircle
                                :circleColor="this.items.minorCategory.filter(item => item.id === minorCategory)[item.id - 1].colorCode" />
                        </div>
                    </div>
                    <div class="minorCategoryItem" v-else>
                        <div v-for="minorCategory, index in item.value" :key="index">
                            <menuText level="level3"
                                :label="this.items.minorCategory.filter(item => item.id === minorCategory)[item.id - 1].name" />
                        </div>
                    </div>
